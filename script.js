// ==================== ИНИЦИАЛИЗАЦИЯ ====================
const API_BASE = 'https://usfinder.bothost.tech';
...
async function startSearch() {
    ...
    const interval = setInterval(() => {
        if (!searchCanceled) {
            attempts++;
            if (counterElement) counterElement.textContent = attempts;
        }
    }, 100);
...
function showResultModal(username, priceRange) {
    ...
    if (resultUsername) resultUsername.textContent = `@${username}`;
    if (resultPrice) resultPrice.textContent = `🔥 Ценность: ${priceRange}`;
...
function initModeSlider() {
    ...
    items.forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (wordField) wordField.style.display = currentMode === 'word' ? 'block' : 'none';
            if (filterField) filterField.style.display = currentMode === 'filter' ? 'block' : 'none';
        });
    });
}
...
async function switchTab(tabId) {
    ...
    if (tabId === 'search') {
        ...
        if (premiumScreen) premiumScreen.remove();
        ...
        <div class="profile-screen" id="profile-screen">
            ...
            <div class="referral-section">
                ...
                <div class="ref-link-box">
                    <input type="text" id="ref-link-input" value="https://t.me/username_finder_bot?start=${userId}" readonly>
