...
# ИМПОРТЫ ДЛЯ ТОЧНОСТИ (Чтобы не было NameError)
...
# Глобальный словарь для хранения клиентов сессий
sessions_clients = {}
sessions_lock = threading.RLock()
loop = None

# Хранилище для временных данных сессии
temp_session_data = {}

# Список используемых клиентов для поиска
...
# Декоратор для блокировки сообщений
...
# Декоратор для блокировки кнопок
...
def refresh_available_clients():
    """Обновляет список доступных клиентов сессий для поиска"""
...
def parse_http_proxy(proxy_str):
    """Парсит HTTP/HTTPS прокси в формат Telethon"""
...
# ========== УЛУЧШЕННАЯ ПРОВЕРКА ЧЕРЕЗ СЕССИИ (РОТАЦИЯ) ==========
def get_available_client():
    """Возвращает случайного клиента из активных сессий"""
    with sessions_lock:
...
async def check_username(client, username):
    try:
        ...
        if not result or len(username) < 5:
            return False
        return True
    except UsernameNotOccupiedError:
        return True
    except (UsernameInvalidError, errors.rpcerrorlist.UsernamePurchaseAvailableError):
        return False
    except Exception:
        return None
...
def check_username_via_session_sync(username):
    ...
    if not client:
        return None
    try:
        # Просто вызываем без проверки loop
...
def check_username_full(username):
    """Улучшенная проверка с приоритетом сессий"""
    
    # 1. Сначала проверяем через сессии (если есть)
    session_result = check_username_via_session_sync(username)
    if session_result is not None:
        return session_result
    
    # 2. Если сессий нет или они не отвечают, используем веб-проверку
    result = check_telegram_fast(username)
    if result is True:
        try:
            r = requests.get(f"https://fragment.com/username/{username}", timeout=2, allow_redirects=True)
            if "query=" in r.url:
                return True
            if "auction" in r.text.lower() or "for sale" in r.text.lower():
                return False
        except:
            pass
        return True
...
def admin_inline_menu():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🔧 Техработы", callback_data="admin_maintenance"),
...
def sessions_menu():
    with sessions_lock:
        ...
        # КД пока нет, добавим позже в БД
        blocked = 0
...
def start(message):
    ...
    if check_subscription(user_id):
        ...
        welcome = (f"<tg-emoji emoji-id='4918354603281482671'>⭐</tg-emoji> ДОБРО ПОЖАЛОВАТЬ!\n\n"
                  ...
                  f"<tg-emoji emoji-id='5118686540985271080'>💎</tg-emoji> <b>Premium от 50 stars</b> — безлимит + всё!\n"
        ...
        for username in batch:
            ...
            if check_username_full(username):
                ...
                win_text = (f"<b><tg-emoji emoji-id='5118590136149345664'>⭐</tg-emoji> Ник найден!</b>\n\n"
                           ...
                           f"<b><tg-emoji emoji-id='4915853119839011973'>🔤</tg-emoji> Осталось поисков:</b> {prem_text}\n"
                ...
                return
        if i % 5 == 0 and msg:
            try: bot.edit_message_text(f"<tg-emoji emoji-id='5134122666331996794'>🔍</tg-emoji> <b>Поиск...</b> {i+1}/{SEARCH_ATTEMPTS}", user_id, msg.message_id, parse_mode='HTML')
            except: pass
        time.sleep(0.05)
    if msg:
...
def process_word_search_new(message):
    ...
    # Популярные доменные зоны и технологические сокращения
    ...
    # Звукоподражания и короткие слова
    ...
    # Глагольные и активные префиксы
...
def profile(message):
    ...
    text = (f"<tg-emoji emoji-id='4904848288345228262'>⭐</tg-emoji> <b>ПРОФИЛЬ</b>\n\n"
            ...
            f"<tg-emoji emoji-id='5116163917713769254'>⭐</tg-emoji> Премиум: {prem_status}\n"
            f"<tg-emoji emoji-id='5116175844837950263'>⭐</tg-emoji> Сегодня поисков: {user.get('searches_today', 0)}/3\n"
...
def profile_back_callback(call):
    # Создаем имитацию message из call
    class FakeMessage:
...
def premium(message):
    ...
    text = (f"<tg-emoji emoji-id='5134122666331996794'>💎</tg-emoji> <b>ПРЕМИУМ ПОДПИСКА</b>\n\n"
            f"<tg-emoji emoji-id='5116275208906343429'>⭐</tg-emoji> ФУНКЦИИ:\n"
            f"<tg-emoji emoji-id='4904819211416633963'>⭐</tg-emoji> Фильтр по маске\n"
            f"<tg-emoji emoji-id='4904819211416633963'>⭐</tg-emoji> Ловушка на ник\n"
            f"<tg-emoji emoji-id='4904819211416633963'>⭐</tg-emoji> Слово\n"
            f"<tg-emoji emoji-id='4904819211416633963'>⭐</tg-emoji> Безлимитный поиск")
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
...
def premium_crypto_callback(call):
    ...
    try:
        ...
        if data.get('ok'):
            ...
            bot.send_message(user_id, f"💳 <b>ОПЛАТА КРИПТОЙ</b>\n\n💰 Сумма: ${price}\n📦 Пакет: {days} дней\n\nНажмите кнопку ниже:", parse_mode='HTML', reply_markup=markup)
...
def admin_give_premium_step(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "❌ Ошибка. Используйте: ID ДНИ", reply_markup=admin_inline_menu())
...
def admin_broadcast_step(message):
    ...
    for user_id, in users:
        ...
        if success % 50 == 0:
            try:
                bot.edit_message_text(f"📢 Рассылка... {success}/{len(users)}", message.chat.id, status_msg.message_id)
            except:
                pass
...
def admin_sessions_menu_callback(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "❌ Нет доступа")
        return
    bot.edit_message_text("🫡 <b>Управление </b>\n\nРобочии сессии {working}", 
...
def session_status_detail(call):
    ...
    with db_lock:
        cursor.execute("SELECT session_name, flood_until FROM sessions WHERE flood_until > datetime('now')")
        flooded = cursor.fetchall()
        blocked_count = len(flooded)
        blocked_list = [row[0] for row in flooded]
    
    text = f"📊 <b>ДЕТАЛИ СЕССИЙ</b>\n\n"
    text += f"✅ Рабочих: {working}\n"
    text += f"⏸ В КД (флуд): {blocked_count}\n"
    ...
    if blocked_count > 0:
        text += "⏸ В КД (ждут разблокировки):\n"
        for s in blocked_list:
            text += f"  • {s}\n"
    
    if working == 0 and blocked_count == 0:
...
def web_app(message):
    ...
    markup.add(types.InlineKeyboardButton(
        "🚀 ОТКРЫТЬ", 
        web_app=types.WebAppInfo(url="http://localhost:8000")
    ))
